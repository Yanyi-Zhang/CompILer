...
class Split_CZSL_Dataset(torch.utils.data.Dataset):
    def __init__(
        self,
        root: str,
        task: int = 0,
        ...
        super(Split_CZSL_Dataset, self).__init__()
        self.train = train
        self.transform = transoform
        self.target_transform = target_transform
        dataset_type = "train" if train else "test"
        dataset_root = Path(root)
        target_file_path = dataset_root / f"{n_tasks}tasks" / f"{dataset_type}_filelist_0{task:d}.txt"
        img_root_path = dataset_root / "images"
        self._load_data(img_root_path, target_file_path)

    def _load_data(self, img_root_path: Path, target_file_path: Path):
